...
instance Monad k => Applicative (StateT s k) where
  ...
  (<*>) skab ska = 
    StateT(\s -> 
      (\(ab, s') -> (\(a, s'') -> (ab a, s'')) <$> runStateT ska s') =<< runStateT skab s
    )
...
instance Monad k => Monad (StateT s k) where
  ...
  (=<<) askb ska =
    StateT(\s -> 
      (\(a, s') -> runStateT (askb a) s') =<< runStateT ska s
    )
...
exec' ::
  ...
  -> s
  -> s
exec' sa s = snd $ runExactlyOne $ runStateT sa s
...
eval' ::
  ...
  -> s
  -> a
eval' sa s = fst $ runExactlyOne $ runStateT sa s
